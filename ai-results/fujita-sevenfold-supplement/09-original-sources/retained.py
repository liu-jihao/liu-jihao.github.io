"""Exact retained-coordinate polynomial engine adapted from the sub7 audit.
Every former assert is an explicit, unconditional check.
"""
CHECKS = 0
CONTEXT = ''
...
def roots(a, l, v):
    require(len(a) <= 2, 'retained arithmetic line 55')
...
def integral(a, l, v):
    return sum((c * (v ** (m + 1) - l ** (m + 1)) / (m + 1) for (m, c) in enumerate(a)), Q(0))
...
def antiderivative(kind, h, B, C):
    ...
    if kind == 'axis':
        ...
        poly = bplus(bpower(x, 5), bscale(btimes(x, bpower(y, 4)), -Q(24, 5) ** 4))
    else:
        raise ValueError(kind)
    return tuple(((i, j + 1, coeff / (j + 1)) for ((i, j), coeff) in sorted(poly.items())))
...
def slice_integral(kind, h, B, C, lower, upper):
    return minus(endpoint_integral(kind, h, B, C, upper), endpoint_integral(kind, h, B, C, lower))

def bernstein(poly, l, v):
    require(len(poly) <= 7, 'retained arithmetic line 141')
    zpower = [sum((poly[m] * comb(m, k) * l ** (m - k) for m in range(k, len(poly))), Q(0)) * (v - l) ** k for k in range(7)]
    result = [sum((zpower[k] * Q(comb(i, k), comb(6, k)) for k in range(i + 1)), Q(0)) for i in range(7)]
    require((v - l) * sum(result) == 7 * integral(poly, l, v), 'retained arithmetic line 148')
    require(result[0] == at(poly, l) and result[6] == at(poly, v), 'retained arithmetic line 149')
    return result

def sign_uniform(a, l, v):
    (lo, hi, mid) = (at(a, l), at(a, v), at(a, (l + v) / 2))
    require(lo >= 0 and hi >= 0 if mid > 0 else lo <= 0 and hi <= 0 if mid < 0 else lo == hi == 0, 'retained arithmetic line 155')
...
def edge(h, B, C, z):
    require(B - C * z > 0, 'retained arithmetic line 169')
...
def interval_polynomial(h, f, B, C, l, v):
    ...
    for (left, right) in zip(edges, edges[1:] + [T]):
        ...
        require(at(left, mid) < at(right, mid), 'retained arithmetic line 214')
    ...
    for (kind, left, right) in [('axis', e1, ehalf), ('middle', ehalf, esixth), ('low', esixth, e0)]:
        ...
        if kind == 'middle':
            ...
            for (aa, bb) in halves:
                ...
                slices.append({'kind': 'convex' if take_convex else 'centroid', 'lower': [str(q) for q in aa], 'upper': [str(q) for q in bb], 'midpoint_centroid': str(at(centroid, mid)), 'midpoint_convex': str(at(convex, mid))})
                poly = plus(poly, scale(selected, 6))
        else:
            slices.append({'kind': kind, 'lower': [str(q) for q in lower], 'upper': [str(q) for q in right]})
            poly = plus(poly, scale(slice_integral(kind, h, B, C, lower, right), 6))
    require(len(choices) in (0, 2), 'retained arithmetic line 248')
